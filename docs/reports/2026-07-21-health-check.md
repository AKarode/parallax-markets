# Parallax Repo Health Check — 2026-07-21

**Status: YELLOW**

Zero source-code commits since the 2026-07-20 check (one tech-research doc — Claude API caching + batch API, DuckDB optimizations, Promptfoo integration). All issues from yesterday carry forward unchanged; no new issues found today.

---

## Summary

433 tests pass, 13 skip, 4 test files fail to collect due to `numpy`/`pandas` absent from the base `[dev]` extras — identical numbers to 2026-07-20. The three standing HIGH issues (`cryptography` missing from `pyproject.toml`, DuckDB single-writer violated across nine files, core simulation/agent infrastructure never built) remain unresolved. No regressions introduced.

---

## Delta From Yesterday (2026-07-20)

| Area | Change |
|------|--------|
| Source code commits | 0 |
| Documentation commits | 1 (tech research: Claude caching/batch API, DuckDB 1.0+ optimizations, Promptfoo eval integration) |
| New issues | 0 |
| Resolved issues | 0 |
...
```
Required for every RSA-PSS-signed Kalshi API request. `cryptography` appears nowhere in `pyproject.toml`. A clean `pip install -e ".[dev]"` will leave the package absent; the first live run hitting Kalshi will fail with `ModuleNotFoundError`. Currently masked by a transitive install from another package.
...
### [LOW] `httpx2` Deprecation Warning in Test Suite

`fastapi`'s `TestClient` emits a `StarletteDeprecationWarning` on every test run:
```
Using `httpx` with `starlette.testclient` is deprecated; install `httpx2` instead.
```
Not a failure, but future FastAPI/Starlette versions may make this an error.

**Fix:** Add `httpx2>=0.28` to `[dev]` extras.
