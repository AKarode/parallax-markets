# Parallax Health Check — 2026-06-24

**Status: YELLOW**

## Summary

No code changes since 2026-06-22 (HEAD remains `37d2f1c`). Tests are unchanged at **17 failed, 416 passed, 13 skipped** — all 17 failures trace to the absent `pytz` dependency (now day 13 unresolved). The `scoring/ledger.py:267` parameter-binding bug and the ten-plus DuckDB single-writer violations also persist unresolved. No new regressions detected.
...
```
HEAD:         37d2f1c  chore: daily health check 2026-06-22 (YELLOW)
Tests:        17 failed | 416 passed | 13 skipped | 1 warning
              (pytz absent system-wide in this env — same failure set as prior runs)
DuckDB:       1.1.x (pip-installed)
Python:       3.11.15
...
- **[HIGH — day 9+] Single-writer pattern violated in 10+ modules**
  ...
  - `ingestion/crisis_ingester.py` — line 81 (crisis_events)
...
- **[HIGH — day 7+] Missing HTTP error handling at market API boundaries**
...
- **[HIGH — day 4] `cryptography>=44.0` conflicts with system package**
...
- **[MEDIUM — carried] Architectural pivot undocumented in spec**
  The live system is a 3-model prediction-market edge-finder; the Phase 1 spec describes a 50-agent LLM swarm with H3 geospatial visualization. `CLAUDE.md` reflects the current design accurately but the spec has not been updated.
...
| `pyproject.toml` dependencies | ✗ Incomplete | `pytz` missing (day 13); `cryptography>=44.0` conflicts with system |
...
| pytz | **NOT INSTALLED** | **MISSING** | Day 13 — 17 test failures in clean envs |
...
1. **Immediate (day 13 overdue)** — Add `"pytz>=2024.1"` to `[project.dependencies]` in `backend/pyproject.toml`. Resolves all 17 failing tests.

2. **Immediate (day 4 overdue)** — Fix `scoring/ledger.py:267`: replace the 3rd list element (`position_id`) with `None` to stop overwriting `trade_id` on every `update_execution()` call.
