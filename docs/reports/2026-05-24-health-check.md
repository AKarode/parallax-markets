# Parallax Health Check — 2026-05-24

**Status: YELLOW**

No code changes since the 2026-05-23 health check — only the daily report was committed. The two HIGH-severity issues (missing `pytz` and invalid Claude model ID) remain unactioned at **39 days** and **26 days** respectively, actively breaking scorecard tests and all live predictions. Starlette bumped 1.0.1 → 1.1.0 and FastAPI 0.136.1 → 0.136.3 in the environment; all 53 endpoint and CLI tests pass against the new versions with no regressions.

---

## Test Results

- **17 failed, 416 passed, 13 skipped** — unchanged for 39 days
- Python 3.11.15 · pytest 8.4.2 · DuckDB 1.5.3 · FastAPI 0.136.3 · Starlette 1.1.0 · anthropic 0.104.1
...
### Starlette 1.0.1 → 1.1.0 / FastAPI 0.136.1 → 0.136.3

Both packages bumped in the environment since the 2026-05-23 check. All 53 endpoint and CLI tests pass against the new versions. No action required.

---

## Issues Found

### [HIGH] `pytz` Missing from `pyproject.toml` — *Open 39 days*
...
### [HIGH] Invalid Claude Model ID Blocks All Live Predictions — *Open 26 days*
...
`ensemble_predict()` (`prediction/ensemble.py:156–169`) scales `confidence` by `penalty_factor` when context is stale but leaves `probability` unchanged. The crisis context was last refreshed ~2026-04-12 (42 days ago), making `penalty_factor` near 0.0. `confidence` is zeroed while `probability` continues to drive divergence detection and trade signals at full strength.
...
`pyproject.toml` specifies only floor versions. Starlette 1.1.0 (minor version bump, no regressions observed) and FastAPI 0.136.3 are installed today. An unreviewed major version could introduce breaking changes without notice.
...
- **416/446 tests pass** — stable for 39 consecutive days
- Starlette 1.1.0 + FastAPI 0.136.3 — all 53 endpoint/CLI tests pass, no regressions
- `anthropic` 0.104.1 — stable
...
1. **`pytz`** — `"pytz>=2024.1"` in `pyproject.toml`. One-line fix. Unblocks 17 tests. **39 days unactioned.**
2. **Model ID** — Replace `"claude-opus-4-20250514"` with `"claude-opus-4-7"` in `oil_price.py`, `ceasefire.py`, `hormuz.py`; update `ensemble.py:89` docstring. Live predictions are broken. **26 days unactioned.**
