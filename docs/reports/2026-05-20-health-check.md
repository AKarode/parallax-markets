# Parallax Health Check — 2026-05-20

**Status: YELLOW**

No commits since the 2026-05-19 health check — all previously identified issues remain open. Test results are unchanged: **17 failed, 416 passed, 13 skipped**. The two HIGH blockers (`pytz` missing and invalid Claude model ID) have been open for 35 and 22 days respectively; live predictions remain broken and 17 tests still fail on fresh installs.

---

## Test Results

- **17 failed, 416 passed, 13 skipped** — identical to 2026-05-19
...
### [HIGH] Invalid Claude Model ID Blocks All Live Predictions — *Open 22 days*
...
### [HIGH] `pytz` Missing from `pyproject.toml` — *Open 35 days*
...
`ensemble_predict()` (`prediction/ensemble.py:156–168`) scales `confidence` by `penalty_factor` when context is stale but leaves `probability` unchanged. At 38+ days stale (crisis context last refreshed ~2026-04-12), `penalty_factor` → 0.0 so `confidence` is zeroed while `probability` continues to drive divergence detection and trade signals at full strength.
...
The spec mandates all writes go through `asyncio.Queue → DbWriter`. These modules write directly:
...
### [LOW] Spec/Architecture Divergence — Prediction Market Pivot
...
- **416/446 tests pass** — no regressions for the 35th consecutive day
...
- `pyproject.toml` requires-python pinned to `>=3.11` (spec said 3.12 — actual runtime is 3.11.15, so this is an intentional correction)
...
| `truthbrush>=0.2` | Added for Truth Social ingestion; not in original spec — intentional addition |
| `cryptography>=44.0` | Added for Kalshi RSA-PSS auth; not in original spec — intentional addition |
...
| Issue | Detail |
|---|---|
| Missing `deck.gl`, `maplibre-gl`, `react-map-gl`, `h3-js` | Listed in CLAUDE.md stack but absent — consistent with the pivot away from H3 visualization |
...
1. **`pytz`** — `"pytz>=2024.1"` in `pyproject.toml`. One-line fix. Unblocks 17 tests. **35 days unactioned.**
2. **Model ID** — Replace `"claude-opus-4-20250514"` with `"claude-opus-4-7"` in `oil_price.py`, `ceasefire.py`, `hormuz.py`; update `ensemble.py:89`. Live predictions are broken without this. **22 days unactioned.**
