# Parallax Health Check — 2026-06-30

**Status: YELLOW**

## Summary

No code changes since 2026-06-22 — now the **8th consecutive stale day**. Tests remain at **17 failed, 416 passed, 13 skipped**, identical to every prior run this streak. The same two critical bugs dominate: the missing `pytz` dependency (day 19) and the `scoring/ledger.py` SQL parameter binding error (day 10). A deeper audit of `portfolio/simulator.py`, `budget/tracker.py`, and `ops/alerts.py` today confirmed additional single-writer violations and a position-closure arithmetic bug that may quietly corrupt portfolio valuations on every live run.
...
```
HEAD:         3883731  chore: daily health check 2026-06-29 (YELLOW)
Tests:        17 failed | 416 passed | 13 skipped | 1 warning
              (failure set unchanged since 2026-06-22)
Code changes: NONE since 2026-06-22 (8th consecutive stale day)
```
...
- **[CRITICAL — day 19] `pytz` absent from `pyproject.toml`**
...
- **[CRITICAL — day 10] `scoring/ledger.py:267` SQL parameter binding bug**
...
- **[HIGH — day 13+] Single-writer pattern violated in 10+ modules**
  `DbWriter` (`db/writer.py`) is correctly implemented but bypassed in all production write paths. Today's deeper audit confirmed the full list:
  ...
  - `budget/tracker.py` — line 43 (llm_usage — synchronous write inside `record()`, called on every LLM call)
  - `ops/alerts.py` — line 106 (blocking `db_conn.execute()` inside `async DuckDBAlertSink.send()`, stalls event loop)
  ...
  - `portfolio/simulator.py` — lines 241, 273, 421, 465 (**newly confirmed** — `_load_signals`, `_load_hit_rates`, `_settle_remaining`, `_update_mark_to_market`)
  Concurrent FastAPI requests and background tasks race on the shared DuckDB connection.

- **[HIGH — new] `portfolio/simulator.py:85` position-closure arithmetic bug**
  When settling a YES position, the code adds `entry_price * quantity` back to `cash` in addition to the payout, doubling capital recovery:
  ```python
  # Bug (line 85) — adds back entry capital incorrectly:
  cash += payout - fees + (pos["quantity"] * pos["entry_price"])
  # Fix — only the payout (minus fees) should be added:
  cash += payout - fees
  ```
  Also, line 93 divides by `pos["quantity"] * pos["entry_price"]` with no guard against zero, risking a `ZeroDivisionError` at portfolio close.
...
- **[MEDIUM — new] Division-by-zero risks in `simulation/cascade.py` and `simulation/config.py`**
  `cascade.py` line 113 divides by `hormuz_daily_flow` without checking for zero. `config.py` line 52 divides by `hormuz_to_europe_via_suez_nm` without a guard. Both raise `ZeroDivisionError` if the YAML scenario sets either value to 0.

- **[MEDIUM — new] `scoring/recalibration.py` crashes on empty calibration bucket**
  `recalibrate_probability()` calls `calibration_curve()`, which returns an empty list if no historical predictions exist. The subsequent bucket lookup has no guard for `bucket_data is None`, so the first live run on a fresh database raises `TypeError`.
  - **Fix:** Add `if bucket_data is None: return (raw_prob, raw_prob)` before line 97.
...
- **[LOW — new] `staleness_penalty_applied` tracked but not used by divergence detector**
  `oil_price.py` logs `staleness_penalty_applied=True` and a `penalty_factor`, but `divergence/detector.py` does not read these fields when computing edge. The penalty is decorative — it never reduces the trade signal strength.
...
| Single-writer topology | ✗ Violated | `DbWriter` exists but 10+ modules bypass it (now including `portfolio/simulator.py`) |
...
| `ledger.py` UPDATE correctness | ✗ Bug | `trade_id` receives `position_id` value on every call (day 10) |
| `portfolio/simulator.py` P&L | ✗ Bug | Position-closure adds entry capital twice; ZeroDivisionError on zero quantity |
| `pyproject.toml` dependencies | ✗ Incomplete | `pytz` missing (day 19); `cryptography>=44.0` blocks fresh install |
...
|---|---|---|
| duckdb | `>=1.2` | OK |
| pytz | **MISSING** | Day 19 — 17 test failures in clean envs |
...
1. **Immediate (day 19 overdue)** — Add `"pytz>=2024.1"` to `[project.dependencies]` in `backend/pyproject.toml`. Resolves all 17 failing tests in one line.

2. **Immediate (day 10 overdue)** — Fix `scoring/ledger.py:267`: replace the 3rd list element (`position_id`) with `None` to stop overwriting `trade_id` on every `update_execution()` call. Signal P&L records are silently corrupted until this lands.

3. **Immediate** — Remove `"cryptography>=44.0"` from `pyproject.toml`. Fresh `pip install -e ".[dev]"` fails entirely on this conflict.

4. **Short-term** — Fix `portfolio/simulator.py:85`: remove `+ (pos["quantity"] * pos["entry_price"])` from the cash-update expression. Add zero-guard before the division on line 93.

5. **Short-term** — Add `try/except (httpx.HTTPStatusError, httpx.RequestError)` around the 5 unguarded methods in `markets/polymarket.py` and the EIA fetch path in `ingestion/oil_prices.py`.

6. **Short-term** — Add `if bucket_data is None: return (raw_prob, raw_prob)` guard in `scoring/recalibration.py` before line 97.

7. **Short-term** — In `cli/brief.py`: (a) add a `None` guard before `.title` on the `active_contracts.get()` result; (b) convert `_persist_run_start()` / `_persist_run_end()` to async and route writes through `DbWriter`.

8. **Medium-term** — Wire `DbWriter` into async-path write sites: `ops/alerts.py` (use `run_in_executor`), `budget/tracker.py`, `scoring/ledger.py`, `scoring/prediction_log.py`, `ingestion/crisis_ingester.py`, `portfolio/simulator.py`.

9. **Medium-term** — Apply `staleness_penalty_applied` / `penalty_factor` inside `divergence/detector.py` so stale-context predictions generate weaker signals.

10. **Medium-term** — Write replacement tests for the 13 skipped mapping-policy scenarios, covering the current fair-value / fee-subtraction architecture.
