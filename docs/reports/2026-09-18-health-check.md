# Parallax Health Check — 2026-09-18

**Status: YELLOW**

Test suite is unchanged at 433 passed, 13 skipped, 1 warning (~169s). No commits have landed since the Sep 17 report; all three HIGH issues are now 7 days old with no fix in sight, which materially increases the risk that a NumPy 2.x environment or concurrent live-pipeline spike causes a production failure.
...
- **No new regressions** since Sep 17
- **No fixes landed:** Only commits since yesterday are the Sep 17 health check and tech research files

---

## Issues Found

### [HIGH] `np.trapz` removed in NumPy 2.x — `scoring/selective.py:106` (open 7 days)
...
### [HIGH] DuckDB single-writer violation — `ops/alerts.py:108` (open 7 days)
...
### [HIGH] 4 bench test files fail to collect — missing import guards (open 7 days)
...
### [MEDIUM] `scoring/tracker.py` + `scoring/ledger.py` — direct DuckDB writes (open 3 days)
...
### [MEDIUM] `backtest/runner.py` — direct DuckDB writes (open 4 days)
...
1. **Fix `np.trapz`** (5 min, 1 line): `scoring/selective.py:106` — `np.trapz` → `np.trapezoid`. Highest-severity, simplest fix; 7 days unfixed.
