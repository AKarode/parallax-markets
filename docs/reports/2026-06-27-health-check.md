# Parallax Health Check — 2026-06-27

**Status: YELLOW**

## Summary

No code changes since 2026-06-22 — now the 5th consecutive stale day. Tests hold at **17 failed, 416 passed, 13 skipped**, unchanged for the fifth straight day. All 17 failures continue to trace to the missing `pytz` dependency (day 16 unresolved). The `cryptography>=44.0` blocker also persists, requiring `--ignore-installed cryptography` to install dev dependencies at all. Two critical bugs — `pytz` in `pyproject.toml` and the `ledger.py` SQL parameter binding error — remain unaddressed on consecutive days without any commit activity.
...
```
HEAD:         2875c2c  chore: daily health check 2026-06-26 (YELLOW)
Tests:        17 failed | 416 passed | 13 skipped | 1 warning
              (same failure set as all prior runs — all trace to missing pytz)
Code changes: NONE since 2026-06-22 (5th consecutive day)
```
...
- **[CRITICAL — day 16] `pytz` absent from `pyproject.toml`**
  `pytz` is not listed in `[project.dependencies]`. DuckDB's Python extension requires it for `TIMESTAMPTZ` columns. In clean environments pytz is absent, causing 17 test failures across `test_scorecard.py` (10), `test_crisis_context_db.py` (4), `test_llm_usage.py` (1), `test_ops_events.py` (1), `test_phase1_critical.py` (1). The `--scorecard` production CLI path is broken in any clean environment. Day 16 with no fix.
...
- **[CRITICAL — day 7] `scoring/ledger.py:267` SQL parameter binding bug**
  `update_execution()` passes `position_id` twice — once into the `trade_id = COALESCE(?, trade_id)` slot and again for `position_id`. Every call silently overwrites `trade_id` with the `position_id` value. Multi-leg orders are mis-recorded in the signal ledger. Day 7 with no fix.
  ```python
...
- **[HIGH — escalated] `cryptography>=44.0` blocks fresh `pip install -e ".[dev]"`**
  `pip install -e ".[dev]"` without `--ignore-installed cryptography` fails with `ERROR: Cannot uninstall cryptography 41.0.7, RECORD file not found` and leaves the package in a broken state where all 42 test modules fail to import. The workaround flag is not in the README, Dockerfile, or any CI config.
...
- **[HIGH — day 11+] Single-writer pattern violated in 10+ modules**
...
| `ledger.py` UPDATE correctness | ✗ Bug | `trade_id` receives `position_id` value on every call (day 7) |
| `pyproject.toml` dependencies | ✗ Incomplete | `pytz` missing (day 16); `cryptography>=44.0` blocks fresh install |
...
| pytz | **NOT INSTALLED** | **MISSING** | Day 16 — 17 test failures in clean envs |
| cryptography | 41.0.7 (system) | `>=44.0` | Blocks `pip install -e ".[dev]"` without `--ignore-installed` |
| fastapi | ✓ | `>=0.115` | OK |
| anthropic | ✓ | `>=0.52` | OK |
| truthbrush | ✓ | `>=0.2` | OK |
...
1. **Immediate (day 16 overdue)** — Add `"pytz>=2024.1"` to `[project.dependencies]` in `backend/pyproject.toml`. Resolves all 17 failing tests in one line.

2. **Immediate (day 7 overdue)** — Fix `scoring/ledger.py:267`: replace the 3rd list element (`position_id`) with `None` to stop overwriting `trade_id` on every `update_execution()` call. Signal P&L records are silently corrupted until this lands.

3. **Immediate** — Remove `"cryptography>=44.0"` from `pyproject.toml`. Fresh `pip install -e ".[dev]"` fails entirely on this conflict, taking all 42 test modules with it. No direct import; purely a transitive dependency.
