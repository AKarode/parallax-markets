# Parallax Health Check — 2026-06-25

**Status: YELLOW**

## Summary

No code changes since 2026-06-22 (HEAD still `39f0fe2`). Tests hold at **17 failed, 416 passed, 13 skipped** — unchanged for the third straight day. All 17 failures trace to the absent `pytz` dependency (day 14 unresolved). The `ledger.py:267` parameter-binding bug (day 5) and the 10+ DuckDB single-writer violations remain open. No new regressions.
...
```
HEAD:         39f0fe2  chore: daily health check 2026-06-24 (YELLOW)
...
- **[CRITICAL — day 14] `pytz` absent from `pyproject.toml`**
  `pytz` is not listed in `[project.dependencies]`. DuckDB's Python extension requires it for `TIMESTAMPTZ` columns. In this environment pytz is absent, causing 17 test failures across `test_scorecard.py` (10), `test_crisis_context_db.py` (4), `test_llm_usage.py` (1), `test_ops_events.py` (1), `test_phase1_critical.py` (1). The `--scorecard` production CLI path is broken in any clean environment. This has been open 14 consecutive days without a fix.
...
- **[CRITICAL — day 5] `scoring/ledger.py:267` SQL parameter binding bug**
...
- **[HIGH — day 8+] Missing HTTP error handling at market API boundaries**
...
- **[HIGH — day 5] `cryptography>=44.0` conflicts with system package**
  System cryptography is `41.0.7` (Debian). Fresh `pip install -e ".[dev]"` fails without `--ignore-installed cryptography`. Blocks standard CI setup.
  - **Fix:** Remove `"cryptography>=44.0"` from `pyproject.toml` — it has no direct import in the codebase (transitive dep only).
...
- **[MEDIUM — carried] 13 mapping-policy tests permanently skipped**
  All 13 are marked `@pytest.mark.skip(reason=_PRE_REFACTOR_REASON)`. The new fair-value / fee-subtraction architecture has no replacement coverage for these contract classes.
...
| `ledger.py` UPDATE correctness | ✗ Bug | `trade_id` receives `position_id` value on every call (day 5) |
| `pyproject.toml` dependencies | ✗ Incomplete | `pytz` missing (day 14); `cryptography>=44.0` conflicts with system |
...
| pytz | **NOT INSTALLED** | **MISSING** | Day 14 — 17 test failures in clean envs |
...
1. **Immediate (day 14 overdue)** — Add `"pytz>=2024.1"` to `[project.dependencies]` in `backend/pyproject.toml`. Resolves all 17 failing tests in one line.

2. **Immediate (day 5 overdue)** — Fix `scoring/ledger.py:267`: replace the 3rd list element (`position_id`) with `None` to stop overwriting `trade_id` on every `update_execution()` call. Signal P&L records are silently corrupted until this lands.

3. **Short-term** — Remove `"cryptography>=44.0"` from `pyproject.toml`. No direct import; conflict blocks all non-Docker installs.
