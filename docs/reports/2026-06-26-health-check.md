# Parallax Health Check — 2026-06-26

**Status: YELLOW**

## Summary

No code changes since 2026-06-22 — HEAD is `80b3da9` (yesterday's health check commit). Tests hold at **17 failed, 416 passed, 13 skipped** — unchanged for the fourth straight day. All 17 failures trace to the absent `pytz` dependency (day 15 unresolved). A new environmental observation: running `pip install -e ".[dev]"` without `--ignore-installed cryptography` now raises an ERROR that breaks all 42 test file imports, confirming the `cryptography>=44.0` conflict is a harder blocker than previously categorized. No new regressions in the test suite itself.
...
```
HEAD:         80b3da9  chore: daily health check 2026-06-25 (YELLOW)
Tests:        17 failed | 416 passed | 13 skipped | 1 warning
              (same failure set as prior runs — all trace to missing pytz)
Code changes: NONE since 2026-06-22
```
...
- **[CRITICAL — day 15] `pytz` absent from `pyproject.toml`**
  `pytz` is not listed in `[project.dependencies]`. DuckDB's Python extension requires it for `TIMESTAMPTZ` columns. In clean environments pytz is absent, causing 17 test failures across `test_scorecard.py` (10), `test_crisis_context_db.py` (4), `test_llm_usage.py` (1), `test_ops_events.py` (1), `test_phase1_critical.py` (1). The `--scorecard` production CLI path is broken in any clean environment. Day 15 with no fix.
...
- **[CRITICAL — day 6] `scoring/ledger.py:267` SQL parameter binding bug**
  `update_execution()` passes `position_id` twice — once into the `trade_id = COALESCE(?, trade_id)` slot and again for `position_id`. Every call silently overwrites `trade_id` with the `position_id` value. Multi-leg orders are mis-recorded in the signal ledger. Day 6 with no fix.
  ```python
...
- **[HIGH — escalated] `cryptography>=44.0` blocks fresh `pip install -e ".[dev]"`**
  Previously categorized as a conflict warning; confirmed today that `pip install -e ".[dev]"` without `--ignore-installed cryptography` fails with `ERROR: Cannot uninstall cryptography 41.0.7, RECORD file not found` and leaves the package in a broken state where all 42 test modules fail to import (42 collection errors instead of 17 failures). This blocks any CI runner or clean Docker build that doesn't manually pass `--ignore-installed`. The flag is not in the README, Dockerfile, or any CI config.
  - **Fix:** Remove `"cryptography>=44.0"` from `pyproject.toml` — it has no direct import in the codebase (transitive dep only, pulled in by `truthbrush`).

- **[HIGH — day 10+] Single-writer pattern violated in 10+ modules**
  `DbWriter` (`db/writer.py`) is correctly implemented but bypassed in production paths. Modules doing direct `conn.execute()` inserts/updates:
  ...
  - `ingestion/crisis_ingester.py` — lines 54, 79 (crisis_events reads + insert)
...
| `ledger.py` UPDATE correctness | ✗ Bug | `trade_id` receives `position_id` value on every call (day 6) |
| `pyproject.toml` dependencies | ✗ Incomplete | `pytz` missing (day 15); `cryptography>=44.0` blocks fresh install |
...
|---|---|---|---|
| duckdb | 1.x (pip) | `>=1.2` | OK |
| pytz | **NOT INSTALLED** | **MISSING** | Day 15 — 17 test failures in clean envs |
| cryptography | 41.0.7 (system) | `>=44.0` | ESCALATED — blocks `pip install -e ".[dev]"` without override flag |
| fastapi | ✓ | `>=0.115` | OK |
| anthropic | ✓ | `>=0.52` | OK |
| truthbrush | ✓ | `>=0.2` | OK |
...
1. **Immediate (day 15 overdue)** — Add `"pytz>=2024.1"` to `[project.dependencies]` in `backend/pyproject.toml`. Resolves all 17 failing tests in one line.

2. **Immediate (day 6 overdue)** — Fix `scoring/ledger.py:267`: replace the 3rd list element (`position_id`) with `None` to stop overwriting `trade_id` on every `update_execution()` call. Signal P&L records are silently corrupted until this lands.

3. **Immediate (escalated)** — Remove `"cryptography>=44.0"` from `pyproject.toml`. Fresh `pip install -e ".[dev]"` now fails entirely on this conflict, taking all 42 test modules with it. No direct import; purely a transitive dependency.
