# Parallax Health Check — 2026-05-26

**Status: YELLOW**

No code changes since the 2026-05-24 health check — only the daily report was committed. The two HIGH-severity issues (`pytz` missing and invalid Claude model ID) remain unactioned at **41 days** and **28 days** respectively, actively breaking scorecard tests and all live predictions. Package versions (FastAPI 0.136.3, Starlette 1.1.0, anthropic 0.104.1, DuckDB 1.5.3) are unchanged; all 416 passing tests continue to pass with no new regressions.

---

## Test Results

- **17 failed, 416 passed, 13 skipped** — unchanged for 41 days
- Python 3.12 · pytest 8.4.2 · DuckDB 1.5.3 · FastAPI 0.136.3 · Starlette 1.1.0 · anthropic 0.104.1
...
Root cause: `TIMESTAMPTZ` columns trigger a DuckDB internal pytz import that fails in this environment. `pytz` is absent from `pyproject.toml`.

---

## New This Run

No code changes. No new issues found. No package updates since 2026-05-24.

---

## Issues Found

### [HIGH] `pytz` Missing from `pyproject.toml` — *Open 41 days*
...
### [HIGH] Invalid Claude Model ID Blocks All Live Predictions — *Open 28 days*
...
`ensemble_predict()` (`prediction/ensemble.py:156–169`) scales `confidence` by `penalty_factor` when context is stale but leaves `probability` unchanged. The crisis context was last refreshed ~2026-04-12 (44 days ago), making `penalty_factor` near 0.0. `confidence` is zeroed while `probability` continues to drive divergence detection and trade signals at full strength.
...
`pyproject.toml` specifies only floor versions. An unreviewed major version bump could introduce breaking changes without notice.
...
The spec modules never implemented include: `agents/runner.py`, `spatial/`, `eval/`, `api/routes.py`, `api/websocket.py`, `api/auth.py`, `simulation/engine.py`, `simulation/circuit_breaker.py`.

---

## Positive Findings

- **416/446 tests pass** — stable for 41 consecutive days
- FastAPI 0.136.3, Starlette 1.1.0, anthropic 0.104.1, DuckDB 1.5.3 — all stable; no regressions
...
1. **`pytz`** — `"pytz>=2024.1"` in `pyproject.toml`. One-line fix. Unblocks 17 tests. **41 days unactioned.**
2. **Model ID** — Replace `"claude-opus-4-20250514"` with `"claude-opus-4-7"` in `oil_price.py`, `ceasefire.py`, `hormuz.py`; update `ensemble.py:89` docstring. Live predictions are broken. **28 days unactioned.**
