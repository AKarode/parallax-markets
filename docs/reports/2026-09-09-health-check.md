# Parallax Health Check — 2026-09-09

**Status: YELLOW**

## Summary

No production code changes since the 2026-09-08 check — commits since the last health report are docs only (tech research + health check). Test suite unchanged: **490 passed, 13 skipped, 1 failed** (`test_selective.py::test_risk_coverage_perfect_ranking` — `np.trapz` removed in NumPy 2.0). All HIGH-severity bugs flagged since 2026-08-26 (14+ days) remain unaddressed. The codebase is architecturally stable but correctness and write-safety defects persist at the same severity as yesterday.

---

## What Changed Since Yesterday

- `96d8148` — Daily tech research report (2026-09-08) (docs only)
- `2b90728` — Daily health check 2026-09-08 (docs only)
...
**Trend:** Core suite stable. One confirmed test failure surfaces `np.trapz` removed in NumPy 2.0. No regressions introduced; no bugs fixed. All 14+ day carry-over bugs remain open.
...
`scoring/selective.py:106` uses `np.trapz(risk, coverage)`, removed in NumPy 2.0 (`AttributeError`). **Active test failure** (`test_selective.py::test_risk_coverage_perfect_ranking`). Runtime crash in NumPy 2.0.
...
`numpy`, `pandas`, and `scikit-learn` are required by `test_bench_forecast.py`, `test_calibration_metrics.py`, `test_recalibrators.py`, and `test_selective.py` but absent from `[project.optional-dependencies.dev]`. Fresh CI installs hit collection errors without manual intervention.

**Fix:** Add `numpy>=1.26`, `pandas>=2.0`, `scikit-learn>=1.3` to `[project.optional-dependencies.dev]`.

### MEDIUM — No Test Coverage for `backtest/runner.py` (Persistent)

`backtest/runner.py` has 4 write methods and an orchestration loop with zero dedicated test coverage. A runner regression is invisible to the test suite.
...
- `simulation/engine.py`, `simulation/circuit_breaker.py`, `agents/`, `spatial/`, `eval/`, `api/websocket.py`, `api/auth.py`
...
**Priority 1 (2-line fixes, unblock correctness — same as prior 14 days):**

1. **Fix TypeError crash** (`scoring/ledger.py:283`): change `int(row[1])` → `int(row[1] or 0)` — prevents silent crash before first resolution cycle.
2. **Fix `np.trapz` → `np.trapezoid`** (`scoring/selective.py:106`): one-line fix, unblocks the active test failure.
...
4. **Add numpy/pandas/scikit-learn to dev deps** in `pyproject.toml` — prevents fresh CI failures.
...
*These three Priority 1 items are each 1–2 line fixes. The same bugs have been open for 14 consecutive days. Addressing them requires less time than reading this report.*
