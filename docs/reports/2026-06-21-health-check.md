# Parallax Health Check — 2026-06-21

**Status: YELLOW**

## Summary

No code changes landed on `main` since the 2026-06-20 report (HEAD is still `5062c2d`). All critical and high-severity issues from prior reports remain unresolved. `pytz` has been missing from `pyproject.toml` for **10 consecutive days**, blocking 17 tests and the production `--scorecard` path. The `ledger.py:267` parameter binding bug silently mis-records `trade_id` on every `update_execution()` call and is now on day 3 without a fix.
...
- **[CRITICAL — day 10] `pytz` absent from `pyproject.toml`**
...
- **[CRITICAL — day 4] DuckDB 1.5.4 breaks H3 community extension**
...
- **[CRITICAL — day 3] `scoring/ledger.py:267` SQL parameter binding bug**
...
- **[HIGH — day 9+] Single-writer pattern violated across 10+ modules**
...
- **[HIGH — day 7+] Missing HTTP error handling in `polymarket.py` and `oil_prices.py`**
...
- **[HIGH — day 3] `cli/brief.py` sync helpers called in async context**
...
- **[HIGH — day 3] Potential `AttributeError` in `cli/brief.py` ticker mapping**
...
|---|---|---|---|
| duckdb | NOT INSTALLED | `>=1.2` | Last known: 1.5.4 — breaks H3 ext (day 4) |
| pytz | NOT INSTALLED | **MISSING** | **Day 10 — breaks 17 tests + production paths** |
| cryptography | 41.0.7 (system) | `>=44.0` | Conflict — blocks fresh `pip install` (day 4) |
...
| `ledger.py` UPDATE correctness | ✗ Bug | `trade_id` column receives `position_id` value on every call (day 3) |
| `conftest.py` H3 fixture | ✗ Broken | Unconditional `INSTALL h3` fails on DuckDB 1.5.4 (day 4) |

---

## Recommendations (Priority Order)

1. **Immediate (10 days overdue)** — Add `"pytz>=2024.1"` to `dependencies` in `backend/pyproject.toml`. Restores 17 failing tests and unblocks `--scorecard` in production.

2. **Immediate** — In `conftest.py`, wrap `INSTALL h3 FROM community; LOAD h3;` in `try/except Exception: pass`. Restores 6 erroring tests.

3. **Immediate (day 3)** — Fix `scoring/ledger.py:267`: pass `None` as the 3rd element in the parameter list (the `trade_id` slot) to stop silently overwriting `trade_id` with `position_id` on every `update_execution()` call.
